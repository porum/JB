...
import io.github.porum.jb.api.DataType
...
private val nativePostCallbacks = ConcurrentHashMap<String, Callback>()
...
private fun handleJsReplyMessage(message: JSONObject) {
  ...
  nativePostCallbacks.remove(id)?.invoke(ResponsePayload(code, data))
}

fun <T> WebView.postMessage(name: String, payload: T? = null, callback: Callback? = null) {
  val id = UUID.randomUUID().toString()
  if (callback != null) {
    nativePostCallbacks[id] = callback
  }
